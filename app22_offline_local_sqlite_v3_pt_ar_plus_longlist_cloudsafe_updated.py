...
def load_longlist() -> pd.DataFrame:
    ...
    Logique :
    1. lecture du CSV s'il existe ;
    2. lecture du XLSX s'il existe ;
    3. fusion intelligente, en donnant priorité aux libellés PT/AR du fichier le plus riche ;
    4. fallback robuste si un seul des deux existe.
    """
    csv_names = ["indicator_longlist.csv"]
    xlsx_names = ["longlist.xlsx", "longlist_filled_pt_ar_final.xlsx"]
    ...
    for p in candidate_paths(csv_names):
        if os.path.exists(p):
            try:
                df_csv = _normalize_longlist_csv(pd.read_csv(p, dtype=str))
                df_csv_path = p
                break
            except Exception:
                continue
    ...
    for p in candidate_paths(xlsx_names):
        if os.path.exists(p):
            try:
                df_xlsx = _normalize_longlist_xlsx(pd.read_excel(p, dtype=str))
    ...
    if df_csv is not None and not df_csv.empty and df_xlsx is not None and not df_xlsx.empty:
        merged = _merge_longlists(df_csv, df_xlsx)
        merged.attrs["source_path"] = f"merged::{df_csv_path} + {df_xlsx_path}"
        return merged

    if df_xlsx is not None and not df_xlsx.empty:
        df_xlsx.attrs["source_path"] = df_xlsx_path
        return df_xlsx
...
def load_countries() -> pd.DataFrame:
    ...
    Prend explicitement en charge COUNTRY_ISO3_4.xlsx.
    """
    candidate_names = [
        "COUNTRY_ISO3_4.xlsx",
    ...
    for p in candidate_paths(candidate_names):
        if os.path.exists(p):
            try:
                df = _normalize_countries_df(pd.read_excel(p, dtype=str))
    ...
    try:
        with st.expander(t(lang, "Diagnostic technique", "Technical diagnostics", "Diagnóstico técnico", "تشخيص تقني"), expanded=False):
            st.write({
                ...
                "app_dir": APP_DIR,
                "longlist_csv_exists": any(os.path.exists(p) for p in candidate_paths(["indicator_longlist.csv"])),
                "longlist_xlsx_exists": any(os.path.exists(p) for p in candidate_paths(["longlist.xlsx","longlist_filled_pt_ar_final.xlsx"])),
                "country_xlsx_exists": any(os.path.exists(p) for p in candidate_paths(["COUNTRY_ISO3_4.xlsx","COUNTRY_ISO3_with_EN.xlsx","COUNTRY_ISO3.xlsx","COUNTRY_ISO3_4_filled_pt_ar_final.xlsx"])),
            })
    except Exception:
        pass
