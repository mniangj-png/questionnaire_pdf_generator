    ...
    try:
        with st.expander(t(lang, "Diagnostic technique", "Technical diagnostics", "Diagnóstico técnico", "تشخيص تقني"), expanded=False):
            st.write({
                ...
                "app_dir": APP_DIR,
                "longlist_csv_exists": any(os.path.exists(p) for p in [LONG_LIST_CSV, "indicator_longlist.csv", os.path.join('.', 'indicator_longlist.csv'), os.path.join('.', 'data', 'indicator_longlist.csv')]),
                "longlist_xlsx_exists": any(os.path.exists(p) for p in [LONG_LIST_XLSX, "longlist.xlsx", os.path.join('.', 'longlist.xlsx'), os.path.join('.', 'data', 'longlist.xlsx')]),
                "country_xlsx_exists": any(os.path.exists(p) for p in [COUNTRY_XLSX, "COUNTRY_ISO3_with_EN.xlsx", os.path.join('.', 'COUNTRY_ISO3_with_EN.xlsx'), os.path.join('.', 'data', 'COUNTRY_ISO3_with_EN.xlsx')]),
            })
    except Exception:
        pass
